...
/**
  ...
  *           @arg @ref CMU_LSCLK_ULRC   32KHz
  *           @arg @ref CMU_LSCLK_LRC_4  32KHz
  *           @arg @ref CMU_LSCLK_LOSC   32768Hz
  */
ald_status_t ald_cmu_lsclk_config(cmu_lsclk_t clk)
{
    uint32_t cnt = 4000U;
    assert_param(IS_CMU_LSCLK(clk));
    MODIFY_REG(CMU->CSR, CMU_CSR_LSCK_CMD_MSK, clk << CMU_CSR_LSCK_CMD_POSS);

    while ((READ_BITS(CMU->CSR, CMU_CSR_LSCK_STU_MSK, CMU_CSR_LSCK_STU_POSS) == clk) && --cnt);

    if (READ_BITS(CMU->CSR, CMU_CSR_LSCK_STU_MSK, CMU_CSR_LSCK_STU_POSS) != clk)
    {
        return ERROR;
    }

    if (clk == CMU_LSCLK_LRC_4)
    {
        SET_BIT(CMU->CLKENR, CMU_CLKENR_LRCEN_MSK);
    }
...
//                MODIFY_REG(MSC->MEMWAIT, MSC_MEMWAIT_FLASH_W_MSK, 0x1 << MSC_MEMWAIT_FLASH_W_POSS);
        ...
        case CMU_CLOCK_HOSC:
            ...
            for (i = 0; (1 << i) <= (clock / 1000000); i++);

            MODIFY_REG(CMU->HOSMCR, CMU_HOSMCR_FRQS_MSK, i - 1);
        ...
        case CMU_CLOCK_LSCLK:
            assert_param(clock <= 1000000);

            if (READ_BIT(CMU->CSR, CMU_CSR_LSCK_STU_MSK) == 0)
            {
                SYSCFG_LOCK();
                return ERROR;
            }

            MODIFY_REG(CMU->CSR, CMU_CSR_SYS_CMD_MSK, CMU_CLOCK_LSCLK << CMU_CSR_SYS_CMD_POSS);

            while (READ_BIT(CMU->CSR, CMU_CSR_SYS_RDYN_MSK) && (--cnt));

            if (READ_BITS(CMU->CSR, CMU_CSR_SYS_STU_MSK, CMU_CSR_SYS_STU_POSS) != CMU_CLOCK_LSCLK)
            {
                SYSCFG_LOCK();
                return ERROR;
            }
...
/**
  ...
  * @retval None
  */
void ald_cmu_pll1_config(uint32_t hosc_clock)
{
    uint32_t cnt = 4000U;
...
void ald_cmu_div_config(cmu_bus_t bus, cmu_div_t div)
{
    ...
    switch (bus)
    {
        ...
        case CMU_PCLK_3:
            MODIFY_REG(CMU->CFGR, CMU_CFGR_PCLK3DIV_MSK, div << CMU_CFGR_PCLK3DIV_POSS);
            break;
...
void ald_cmu_buzz_config(cmu_buzz_div_t div, uint16_t dat, type_func_t status)
{
    ...
    if (status)
    {
        ...
        MODIFY_REG(CMU->BUZZCR, CMU_BUZZCR_BUZZDAT_MSK, dat << CMU_BUZZCR_BUZZDAT_POSS);
...
void ald_cmu_lptim0_clock_select(cmu_lp_perh_clock_sel_t clock)
{
    ...
    SYSCFG_UNLOCK();
    MODIFY_REG(CMU->PERICR, CMU_PERICR_LP16TCS_MSK, clock << CMU_PERICR_LP16TCS_POSS);
    SYSCFG_LOCK();
...
void ald_cmu_qspi_clock_select(cmu_qspi_clock_sel_t clock)
{
    ...
    SYSCFG_UNLOCK();
    MODIFY_REG(CMU->CFGR, CMU_CFGR_QSPIRCS_MSK, clock << CMU_CFGR_QSPIRCS_POS);
    SYSCFG_LOCK();
