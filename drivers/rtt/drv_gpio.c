...
void es32w3_pin_write(rt_device_t dev, rt_base_t pin, rt_base_t value)
{
    const struct pin_index *index;
    index = get_pin(pin);
    if (index == RT_NULL)
...
int es32w3_pin_read(rt_device_t dev, rt_base_t pin)
{
    int value;
    const struct pin_index *index;
    value = PIN_LOW;
...
void es32w3_pin_mode(rt_device_t dev, rt_base_t pin, rt_base_t mode)
{
    const struct pin_index *index;
...
rt_err_t es32w3_pin_attach_irq(struct rt_device *device, rt_int32_t pin,
...
rt_err_t es32w3_pin_detach_irq(struct rt_device *device, rt_int32_t pin)
{
    const struct pin_index *index;
    rt_base_t level;
...
rt_err_t es32w3_pin_irq_enable(struct rt_device *device, rt_base_t pin,
                               rt_uint32_t enabled)
{
    const struct pin_index *index;
...
const static struct rt_pin_ops _es32w3_pin_ops =
{
    es32w3_pin_mode,
    es32w3_pin_write,
    es32w3_pin_read,
    es32w3_pin_attach_irq,
    es32w3_pin_detach_irq,
    es32w3_pin_irq_enable
};
...
int rt_hw_pin_init(void)
{
    ...
    result = rt_device_pin_register(ES_DEVICE_NAME_PIN, &_es32w3_pin_ops, RT_NULL);
