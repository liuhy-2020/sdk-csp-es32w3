...
const struct rt_spi_ops es32w3_spi_ops =
{
    spi_configure,
    spixfer,
};

rt_err_t es32w3_spi_device_attach(rt_uint32_t pin, const char *bus_name, const char *device_name)
{
    int result;
    /* define spi Instance */
    struct rt_spi_device *spi_device = (struct rt_spi_device *)rt_malloc(sizeof(struct rt_spi_device));
    RT_ASSERT(spi_device != RT_NULL);
    struct es32w3_hw_spi_cs *cs_pin = (struct es32w3_hw_spi_cs *)rt_malloc(sizeof(struct es32w3_hw_spi_cs));
...
int rt_hw_spi_init(void)
{
    ...
    result = rt_spi_bus_register(spi_bus, ES_DEVICE_NAME_SPI0_BUS, &es32w3_spi_ops);
    if (result != RT_EOK)
    {
        return result;
    }
    
    result = es32w3_spi_device_attach(ES_SPI0_NSS_PIN, ES_DEVICE_NAME_SPI0_BUS, ES_DEVICE_NAME_SPI0_DEV0);
    ...
    spi_bus->parent.user_data = spi;
    result = rt_spi_bus_register(spi_bus, ES_DEVICE_NAME_SPI1_BUS, &es32w3_spi_ops);
    if (result != RT_EOK)
    {
        return result;
    }
    
    result = es32w3_spi_device_attach(ES_SPI1_NSS_PIN, ES_DEVICE_NAME_SPI1_BUS, ES_DEVICE_NAME_SPI1_DEV0);
