...
static rt_err_t es32w3_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
 ...
 * Setup DMA
 */
static rt_err_t es32w3x_dma_config(struct rt_serial_device *serial, rt_ubase_t flag)
{
    struct es32_uart *uart;
...
static rt_err_t es32w3_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    struct es32_uart *uart;
#ifdef RT_SERIAL_USING_DMA
    ...
    case RT_DEVICE_CTRL_SET_INT:
        ...
        case RT_DEVICE_CTRL_CONFIG:
            /* Setup DMA */
            es32w3x_dma_config(serial, ctrl_arg);
            break;
#endif
...
static int es32w3_putc(struct rt_serial_device *serial, char c)
{
    struct es32_uart *uart;
    RT_ASSERT(serial != RT_NULL);
    uart = (struct es32_uart *)serial->parent.user_data;
...
static int es32w3_getc(struct rt_serial_device *serial)
{
    int ch = -1;
    struct es32_uart *uart;
 ...
 * DMA transmit
 */
static rt_size_t es32w3x_dma_transmit(struct rt_serial_device *serial, rt_uint8_t *buf, rt_size_t size, int direction)
{
    struct es32_uart *uart;
...
static const struct rt_uart_ops es32_uart_ops =
{
    es32w3_configure,
    es32w3_control,
    es32w3_putc,
